...
4. Release típico: `git pull` → `npm ci` → `env:check` se necessário → `db:migrate` se necessário → `build` → restart seletivo → smoke (`/empresarial/api/saude`, `.status`/`.total_registros` em `/empresarial/api/rals`).
