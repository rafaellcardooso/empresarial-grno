...
Manaus (MNS): Xpertrak `http://200.160.100.6/pathtrak/api`, 19 CMTS em `config/cities/mns.json`. **CASA:** L2VPN via SNMP em `docsL2vpnVpnCmTable` (`1.3.6.1.4.1.4491.2.1.8.1.4.1.1`); a tabela NSI (`1.9.1.2`) costuma estar vazia. Sonda VLAN: `scripts/snmp_probe_bsod.py`.
...
3. SNMP L2VPN + **ping ICMP** + LDAP → `bsod_inventory`:
   - VLAN L2VPN por CMTS (`bsod_vlan`);
   - ping (3 tentativas) quando PathTrak offline + IP PME (`ping_reachable`);
